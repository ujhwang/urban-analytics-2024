...
MARTA_buffer <- route.shape %>% 
  sf::st_transform(crs = ????) %>% # What was the EPGS code for WGS 84 / UTM zone 16N?
...
MARTA_stop_buffer <- gtfs_sf$stops %>% 
  st_transform(crs = ????) %>%  # What was the EPGS code for WGS 84 / UTM zone 16N?
...
acs2022 <- tidycensus::get_acs(geography = "tract",
...
acs2022 <- acs2022 %>% 
  mutate(pct_no_car = no_car_hhE/total_hhE) %>% 
  # Calculate area of the Census Tract polygons
  mutate(area = st_area(.) %>% unclass) %>% 
  select(GEOID, hhinc = hhincE, pct_no_car, area) %>% 
  filter(!is.na(hhinc))


pal1 <- colorNumeric(palette = "YlOrRd", domain = acs2022$hhinc)
...
leaflet() %>% 
  ...
  addPolygons(group = "ACS",
              ...
              fillOpacity = 0.5,
              weight = 1, 
              popup = leafpop::popupTable(round(st_drop_geometry(acs2022[,c("hhinc", "pct_no_car")]),2))) %>% 
  addPolygons(group = "MARTA",
              data = MARTA_buffer_group %>% 
                st_transform(crs = st_crs(acs2022)), 
...
buffer_intersect_tract <- acs2022 %>% 
  # Unify the CRS
...
leaflet() %>% 
  addProviderTiles(providers$CartoDB.DarkMatter) %>% 
  addPolygons(data = acs2022 %>% st_transform(crs = 4326), fillOpacity = 0.2, color = "white", weight = 0.5, group = 'Census') %>% 
...
# Syntax 2: using pipe and 'with', useful when you want to do 
# something first and then do cor.test with the result
buffer_by_tract %>%
...
am_stop_time <- filter_stop_times(gtfs_obj = gtfs, 
                                  extract_date = "2024-10-14",
                                  min_departure_time = 3600*7, # input unit is in second. So 3600*7 is 7AM
...
On average, you can see that it takes `r round(mean(trvt$travel_time)/60,1)` minutes from the Midtown Station to other stations.
